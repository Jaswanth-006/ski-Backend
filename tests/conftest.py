...
async def _purge_users(session: AsyncSession, ids: Sequence[uuid.UUID]) -> None:
    ...
    await session.execute(delete(StockLoad).where(StockLoad.created_by.in_(ids)))
    ...
    await session.execute(delete(DeliveryOtherSales).where(DeliveryOtherSales.delivery_id.in_(ids)))
